...
Agent = `opencode-ai@1.17.3` sidecar；IPC = 私有包 `@oomol/connection`；
...
5. **版本钉死，禁止浮动**：`opencode-ai` / `@opencode-ai/sdk` / `@opencode-ai/plugin`
   三包同为 `1.17.3`（上游无 API 稳定承诺）；oo CLI 版本由 `scripts/oo-cli.ts` 的
   `OO_CLI_VERSION = "1.2.0"` 单一锁定。
...
- LLM：OOMOL OpenAI 兼容网关 `llm.<endpoint>/v1`，模型名固定 `oomol-chat`，
  provider id `oomol`，agent 名 `lumo`（`electron/agent/config.ts`）。
  网关 `/v1/models` **不会列出** `oomol-chat`（网关侧别名），勿据此"纠正"模型名。

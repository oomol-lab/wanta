...
オプションのホスト型機能は、保存済み Provider 認証情報をエージェントに渡すことなく、管理されたアカウント接続とチームワークスペースを追加します。
チームは接続と Skills を共有し、Provider アクセスと使用量を管理できるため、ID、OAuth 認証情報、ガバナンス基盤を自前で運用する必要がありません。

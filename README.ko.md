...
선택형 호스팅 기능은 저장된 Provider 자격 증명을 에이전트에 전달하지 않으면서 관리된 계정 연결과 팀 워크스페이스를 추가합니다.
팀은 연결과 Skills를 공유하고, Provider 접근 권한과 사용량을 관리하며, 인증·OAuth 자격 증명·거버넌스 인프라를 직접 운영하지 않아도 됩니다.

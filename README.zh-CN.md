...
> **TODO：在此添加一段 20–30 秒的产品视频或 GIF。** 展示用户请求 Wanta 完成任务、
> Agent 使用本地或连接工具、权限/工具活动步骤，以及最终文件在产物面板中打开的过程。
...
> **TODO：在此添加一张主聊天视图的宽幅截图。** 展示真实的多步骤任务、可见的工具活动、
> 侧边栏和产物面板。
...
> **TODO：添加一张展示本地访问权限卡片或结构化问题提示的截图。**
...
> **TODO：添加一张产物面板预览精美输出的截图。** 电子表格工作簿或生成的 PDF 比纯文本
> 文件更具展示力。

### 连接账户，而不把凭据交给 Agent

托管连接体验支持 OAuth2、API Key、自定义凭据、联合凭据和无需认证的 Provider。
一个工作区可以为同一 Provider 保存多个账户，而 Agent 会通过结构化工具识别并使用所选连接。

> **TODO：添加一张连接目录及某个包含多个账户的 Provider 详情页截图。**

### 与团队一起组织工作

登录 Wanta 后，团队可以管理工作区、成员、共享连接、每位成员的 Provider 访问权限、
团队 Skills、用量、订阅和席位。托管方案适合不希望自行运营身份认证、OAuth 凭据和治理
基础设施的开发者与团队。

> **TODO：添加一张展示成员和 Provider 访问控制的团队管理截图。**

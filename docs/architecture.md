...
- **Main process** `electron/main.ts`: the assembly root. It creates
  ...
  / `session` / `skill` / `models` / `settings` / `auth` / `update` / `git` /
...
it as a renderer asset). In the main-process build, AI SDK packages, `@opencode-ai/sdk`,
`electron-updater`, `fsevents`, and `playwright-core` are **externalized** so Node resolves their
...
Team scoping is persisted rather than baked into the sidecar: the manager
atomically writes `teamName` and `sessionTeams` (the team per OpenCode session) to
`userData/agent/team-scope.json` (the tools read it by `sessionID` via
...
/ `git` / `link-runtime` / `browser` (connector and teams each once had a service; after their requests moved to the
...
electron/
  ...
  settings/ common,node,store(+test)  themeSource + completion-notification condition + notification sound + unread badge; atomic writes; stores no credentials (R8); updateChannel lives only in the on-disk store layer, consumed by UpdateService
...
src/
  ...
  components/app-shell/       AppShell three-pane + internal route state (archived|billing|chat|connections|teams|skills|settings)
  ...
  routes/  Archived/ Billing/ Chat/ Connections/ Login/ Settings/ Skills/   (Chat/voice-asr.ts renderer-side speech-to-text, §4)
  hooks/   useChat useSessions useConnections useLinkRuntime useAuth useAppUpdate useBillingOverview useTeamWorkspace useAttention useProjectGit useTeamSkills useAppSettings … (~20 hooks)

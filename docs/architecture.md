...
仅基于命令前已有的会话证据生成 `wanta-bug-report.md`。命令强制使用 Build agent，以便把这一份 UTF-8 Markdown
报告写入该轮托管输出目录；不得调查、重试或修复原问题，也不得调用连接器、网络、shell 或读取额外文件。报告经
上述 `ArtifactBundle` 链路展示为单文件制成品，assistant 正文只给简短完成状态，不重复报告内容。

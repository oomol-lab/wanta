...
| `system-prompt.ts`    | `LUMO_SYSTEM_PROMPT`（英文）：三段式——Connector tools（search→inspect→call 流程）/ Local tools（bash、文件、脚本；cwd 是私有 scratch，访问真实文件用绝对路径或 `~`）/ Operating contract 7 条（含 inspect-before-call、authorization_required 硬停止、最小载荷、副作用确认等）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |

...
| 域            | 渲染层落点                                                                                                                                                          | 主进程残留                                                                                                      |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| billing       | `billing-client.ts`（组织订阅、席位、余额与用量的 insight/console-server 请求 + 结账 URL 解析）；主进程 `chat/billing.ts` 已删                                               | open\* 改 `chatService.openExternalUrl` IPC（仅 `shell.openExternal` 校验外开）                                 |

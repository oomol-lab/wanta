...
| 模块                  | 职责                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `manager.ts`          | `AgentManager`：编排 sidecar。`promptStreaming()`（`session.promptAsync` 非阻塞，agent 走 OpenCode 原生 `build` / `plan`；默认 Build，Planning 传 `agent:"plan"`；非 `default` 且被当前模型支持的 `reasoningLevel` 透传为 OpenCode `body.variant`；OOMOL runtime 默认传 Auto，本地 runtime 默认传选中的 custom model；用户选择 GPT 5.5 时传 `{providerID:"openai", modelID:"gpt-5.5"}`），每轮把 `buildAuthorizedSystem()` 注入 `body.system`——R4 已授权 Link provider 存在性提示，实测为**追加**到 agent.prompt 之后而非覆盖；默认不列具体 provider 名，避免可用性上下文变成工具诱导）；`sendMessage()`（阻塞，headless 用）；`subscribe()`（OpenCode 全局 SSE 事件循环）；session CRUD；`listAuthorizedServices()` 仅在 OOMOL runtime 直查 `${connectorBaseUrl}/v1/apps`。                                                                                                                                                                                                                                                                                  |
| `sidecar.ts`          | `OpencodeSidecar`：spawn `opencode serve --hostname=127.0.0.1 --port=0`，解析 stdout 中 "listening on URL" 拿地址。**禁止加 `--pure`**（会跳过自定义插件，`.opencode/tools` 连接器工具会静默失效）。配置经 `OPENCODE_CONFIG_CONTENT` 环境变量内联注入（OOMOL runtime 的 builtin provider 使用会话 token；local runtime 只含 custom provider Key；均只入内存 env，不落盘）；`OPENCODE_CONFIG_DIR` / `XDG_CONFIG_HOME` / `XDG_DATA_HOME` 指向 `userData/agent/isolation`（隔离全局 `~/.config/opencode`；目录须在启动前异步预建，缺失会 500）；`OPENCODE_DISABLE_EXTERNAL_SKILLS=1`，sidecar 不直接扫 `~/.agents` / `~/.claude` 等全局根，外部 agent skill 由 `SkillServiceImpl` 扫描后同步到 Wanta 私有 workspace，避免同名旧副本抢占；`PATH` 由 `command-path.ts` 按平台合并：macOS/Linux 读取用户登录 shell PATH，Windows 读取当前用户与系统注册表的最新 Path，并在两端都保留 Wanta 自带 bin、Electron 继承 PATH 与平台专属常见命令目录；只导入 PATH，不导入 shell/注册表中的其他环境变量；随机 `OPENCODE_SERVER_PASSWORD` Basic Auth；`dispose()` SIGTERM。 |
| `config.ts`           | `buildOpencodeConfig()`：OOMOL runtime 才生成 provider `oomol`（默认 Auto/`oopilot`，npm `@ai-sdk/openai-compatible`，baseURL=`llmBaseUrl`，同时承载其他 OpenAI-compatible 内置模型）+ provider `openai`（GPT 5.5，OpenAI Responses runtime）；local runtime 只生成 custom provider。注意：网关 `/v1/models` **不会列出** `oopilot`，它是网关侧别名、由 chat/completions 路由到真实模型——勿据 models 列表"纠正"Auto 模型名。覆盖 OpenCode 原生 agent `build` / `plan`：提示由 runtime capability 选择 local/OOMOL Build/Plan 版本；两种 runtime 的 `external_directory`、`edit` 和普通 `bash` 都先走 ask，再由 ChatService 主进程本地访问策略处理。只有 OOMOL runtime 的直接 `oo` / `$WANTA_OO_BIN` / `${WANTA_OO_BIN}` 命令保留快速放行；local runtime 的 bash 一律从 ask 进入同一安全闭环。Plan 的 `edit` 仅允许 `.opencode/plans/*.md`。根级 permission 同步 Build 权限；不下发 tools 禁用表（内置工具全部启用）。                                                                                                                                         |
| `system-prompt.ts`    | 英文 task-first 提示按 core/local/knowledge/connector/output/plan 等 Markdown section 维护，`buildWantaSystemPrompt()` / `buildWantaPlanSystemPrompt()` 根据 Connector capability 组合。local 版本保留本地工具、具体 URL、WikiGraph 和安全契约，但移除 Link 路由、四工具契约、账号授权与跨服务数据传递说明；OOMOL 版本保留 search→inspect→call、inspect-before-call、authorization_required、workspace identity、最小访问/载荷和副作用确认等 Connector 契约。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `tool-sources.ts`     | 5 个自定义工具的 **String.raw 内嵌源码**（`list_apps.ts` / `search_actions.ts` / `inspect_action.ts` / `call_action.ts` / `query_knowledge.ts`），导出 `AGENT_TOOL_FILES`；`agentToolFilesForRuntime()` 在 local 只返回 `query_knowledge.ts`，OOMOL 返回全部工具。工具从 workspace 私有的 `../runtime/tool.js` 导入构建期合并好的 tool helper + Zod，工具加载不依赖 OpenCode 首启联网安装 npm 包。四个 Connector 工具运行于 OpenCode 的 Bun，不参与本项目 tsc/oxlint；经 `execFile` 调 oo，并保留 team selector、授权 canary、限流/熔断和结构化 authorization error 语义。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `workspace.ts`        | `ensureAgentWorkspace(rootDir, bundledSkillsDir?, bundledToolRuntimePath?, cloudRuntime?)`：每次启动按 runtime 幂等重建 `<userData>/agent/workspace/.opencode/tools/*.ts` 并同步构建期合并的 `.opencode/runtime/tool.js`；OOMOL 写入全部五个自定义工具并以 `resources/skills` 为准重建 `.opencode/skill/<name>/` 的 bundled oo Skills，local 只写 `query_knowledge.ts` 并清除 `.opencode/skill/`，因此 runtime 切换不会遗留 Connector 能力。`.opencode/skills/` 是 Wanta registry/runtime Skill 私有目录，由 `SkillServiceImpl` 写入，两种 runtime 都保留且不被 bundled skill 同步清空。workspace 即 sidecar 的 cwd，**不可更改**。                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| billing       | `billing-client.ts` (team subscription/seats, team usage, the creator's personal balance and personal-usage promo-subscription insight/console-server requests + checkout URL resolution); team usage carries a workspace header, the creator's balance/usage subscription carries no team header, and members must not degrade into querying or subscribing to their own personal account; the main-process `chat/billing.ts` is deleted              | `open*` now uses the `chatService.openExternalUrl` IPC (only `shell.openExternal` validates the external open)                |
| voice ASR     | `routes/Chat/voice-asr.ts` (audio is already recorded in the renderer, avoiding a base64 trip through IPC)                                                                                                                                                                                                                                                                                                                                             | none                                                                                                                          |
| teams         | `teams-client.ts` + `team-details-resource.ts` (members, authorizations, provider options as a short-lived shared resource); **the whole IPC service is deleted**                                                                                                                                                                                                                                                                                      | none                                                                                                                          |
...
<<<<<<< HEAD
凭证唯一性带来一致的 OOMOL 生命周期：`AuthManager.currentState()` 异步、**token 门控**——profile 仍在但 cookie 过期/被驱逐
即判为未登录 → 渲染层保留 AppShell 并切到 local workspace；Connector、团队、用量、云 Skills 和语音能力关闭，
有 custom model 时继续 local Agent，无模型时进入 `model_required` 引导。`activeRuntimeAccount()`
...
<<<<<<< HEAD
渲染层 `src/App.tsx`：AuthGate 只等待 auth 与无凭证 runtime capability 两个事实源；就绪后无论是否登录都进入
AppShell，key 使用 `local | account.id` + runtime mode，身份/runtime 变化时整树重挂载。未登录时
`useTeamWorkspace` 产生稳定 Local workspace，Session/Project 使用 `local:local` scope；`model_required` 空状态提供
BYOK 配置和可选 OOMOL 登录。Sidebar、route、模型清单、语音、Billing 和 Connector 空状态均按 capability
裁剪，云 route 在渲染前回退聊天页，避免用 401 探测能力。`LoginRoute` 保留，但不再充当启动门禁。

runtime 交接顺序同样属于安全边界：登录/换号先广播临时未登录状态，让 Renderer 卸载旧云 scope 并清缓存，
随后持久化新 Cookie、经 `applyChain` 回收旧 sidecar 并启动新 Agent，最后广播新账号；登出/expiry 先撤掉云 UI，
再清 Cookie 和应用 local runtime。`AgentManager.dispose()` 同时中止 generation 事件流、授权请求以及启动中或运行中的
sidecar，因此不会有两个 runtime 共享同一 agent root。runtime 回退错误不阻断未登录状态广播；Renderer 会保留
local workspace 并展示 Agent 恢复错误，session/project 数据不随账号切换删除或迁移。
=======

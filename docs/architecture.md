...
holds four packages — `@opencode-ai/sdk`, `diff` (9.0.0), `electron-updater`, and `react-diff-view`
(3.3.3) — but only `@opencode-ai/sdk` and `electron-updater` are externalized. `diff` (imported at `electron/git/turn-diff.ts`)

...
这个架构方向本身合理：问题报告需要确定性触发、可信的 Wanta 运行时元数据、受控的制成品目录和严格的工具边界，这些职责不适合完全交给模型自行判断是否加载某个 Skill。但是当前交互没有把“命令”和“Skill”的差异解释清楚，也没有提供命令已生效的结构化反馈，因此用户很容易形成以下判断：

1. 从与 Skill、Connections 混排的 `/` 菜单选择“问题报告”，自然会以为它也是一个 Skill。
2. 选择后只看到输入框里的 `/bug-report` 文本，没有命令胶囊、参数提示或执行状态，像是普通文本补全。
3. 执行时没有 `skill` tool call；Wanta 现有的 Skill 活动 UI 只会识别 `Loaded skill: ...`，所以用户无法从执行详情确认“问题报告工作流”是否被加载。

推荐方案不是“为了显示 Skill UI 而强行调用 Skill”，而是把它明确建设成一个**结构化的 Wanta Command**：选择时显示“Bug 图标 + 问题报告”胶囊，不额外显示“命令”字样；提交时传递可信的 command id，运行时显示清楚的报告生成状态，结果继续使用 Markdown 制成品卡片。报告规范可以沉淀为单一、可版本化的工作流定义；如果未来需要跨 Agent 复用，再从该定义导出 Agent Skills 标准版本，但 Wanta 内仍由命令适配层负责确定性触发和安全边界。
...
`src/routes/Chat/useComposerPalette.ts` 在用户选择该项后调用 `slashCommandDraftReplacement()`；`src/routes/Chat/composer-palette-logic.ts` 返回字面量 `/bug-report `。因此选择动作只完成了文本替换，不会：

- 添加 `ChatContextMention`；
- 显示 `ContextMentionChips`；
- 自动提交；
- 调用 OpenCode command API；
- 调用 OpenCode `skill` tool。

这正是截图一中只看到 `/bug-report` 普通文本的原因。
...
`/bug-report` 两条路径都没有走，所以当前 UI 不显示 Skill 是符合代码现状的，不是偶发渲染故障。
...
菜单虽然标注了 `command`，但它与 Skill 混排，选中后又退化为普通文本。用户无法确认这是产品能力、prompt macro、Skill，还是仅仅插入了一段建议文本。

#### P0：命令识别依赖用户可编辑文本

UI 已经知道用户选择了 `bug-report`，但提交后主进程只能重新解析 `req.text`。用户只要改动命令拼写、在前后加入其他文本，或者未来本地化命令文本，就可能悄悄退回普通聊天，而 UI 没有明确报错。

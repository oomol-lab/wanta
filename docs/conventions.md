...
- `list_apps` 只回答连接清单或校验用户显式选择的账号，不得作为普通 Link 读取/动作的前置健康检查。每轮动态 system 必须写明该轮 workspace；自定义 Link 工具按 session 自动应用，裸 `oo connector` CLI 若不可避免则必须显式传同一 `--organization` / `--personal`，错误后禁止省略 selector 或切换身份重试。

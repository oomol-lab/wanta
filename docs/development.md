...
- `LUMO_ENDPOINT`：endpoint 主域，缺省 `oomol.com`，对接开发环境改 `oomol.dev`。**读取规则**（`vite.config.ts` 的 `resolveOoEndpoint`）：dev 与 vitest 经 `loadEnv` 读 `.env(.local)`；**build 刻意不读文件**（防开发域名进发布包）；两种模式都尊重显式环境变量，内部联调包用 `LUMO_ENDPOINT=oomol.dev npm run build`。已知坑：`oomol.dev` 的 LLM 网关曾对 Auto/`oopilot` 返回 403 "Model disabled"（后端限制，非代码问题），dev endpoint 主要用于 connector 联调，聊天 403 时先怀疑网关侧。

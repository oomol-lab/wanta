...
- 提代码必须走临时分支 + PR：先把本地 `main` 对齐 `origin/main`，再从 `main` 拉一次性分支（如 `codex/<task>`、`ci/<task>`、`fix/<task>`）。改动完成并通过质量门后推送临时分支，开 PR 到 `oomol-lab/wanta:main`，由 PR 合并回 `main`。不要直接在 `main` 上提交或推送。PR 合并后同步最新 `main`，删除本地临时分支，并删除远端上的同名临时分支。所有 Git 操作中的人类可读文本必须用英文，包括 commit message、branch name、PR title、PR description、PR review/comment、tag/release note。

...
- **UI 实机验证旁路**（dev 专用 env，生产无害）：`VITE_WANTA_SMOKE`（AppShell 就绪后自动发一条消息，`AppShell.tsx`）、`VITE_WANTA_ROUTE=settings`（也支持 `knowledge` 等 AppShell 页面）、`VITE_WANTA_LOCALE`（强制 locale，`src/i18n/i18n.ts`）；配合 macOS `screencapture` 截图取证。

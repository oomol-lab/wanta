...
- **UI 实机验证旁路**（dev 专用 env，生产无害）：`VITE_WANTA_SMOKE`（AppShell 就绪后自动发一条消息，`AppShell.tsx`）、`VITE_WANTA_ROUTE=settings`（也支持 `knowledge` 等 AppShell 页面）、`VITE_WANTA_LOCALE`（强制 locale，`src/i18n/i18n.ts`）；配合 macOS `screencapture` 截图取证。Electron 42 的 macOS 原生通知要求有效应用签名，`.electron-dist/Electron.app` 的 ad-hoc dev 签名不能用于通知验收；设置页会标明并禁用测试通知。请用 CI 签名产物或本机有效 Apple Development / Developer ID 签名的 packaged app 覆盖：允许/拒绝通知；测试结果是否能在 `Notification.getHistory()` 中确认唯一 ID；通知中心已送达但横幅受专注/勿扰或屏幕共享策略抑制时的文案；声音；前台/非前台任务条件；点击回任务路径。注意“已送达通知中心”仍不等于用户一定看到横幅。
- **已知验证缺口**（据会话记录从未实机跑通，排查时勿默认其已验证）：权限确认 UI 下的 bash/edit/external_directory 实调、ai-elements 迁移后的实机视觉效果（当时无显示器环境）、真实账号的浏览器登录回跳（需真人登录）。
...
| `resources/agent-tool-runtime/` | 自定义工具的自包含 runtime（→ extraResources） | `scripts/build-agent-tool-runtime.ts`      |

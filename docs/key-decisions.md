...
- **后果**：当前安全姿态从"任意 shell / 文件读写 / 网络访问全无确认"收敛为"默认访问下 bash 和普通文件能力顺滑可用，只在真实风险边界暂停确认"。用户不需要为 `oo ... | head`、`npm test`、`rg`、数据处理脚本、普通桌面/下载目录文件等常规工作逐次批准；`npm install`、读取 `.ssh`/`.env`、删除、提权、推送、部署等仍需确认。为避免生成文件等任务因 Python 缺依赖连续审批，Agent 必须在当轮 process 目录下创建私有 `.wanta-python` venv；用户可以只对该 venv 中的纯 PyPI 包名安装授予一次任务级 grant，后续同目录且不新增顶级包的重试会自动批准。`--user`、`--break-system-packages`、额外索引、URL/本地路径/requirements 文件和任意系统 Python 安装仍逐次确认。若将来继续细化敏感路径（如浏览器 profile、邮件数据库、更多凭证目录）或外部副作用分类，需要同步 `config.ts`、ChatService 本地访问策略、访问模式 UI、事件测试和 [conventions.md §7](conventions.md)。若将来重新收紧权限：OpenCode permission **只闸内置工具**，`bash: deny` 不会切断 `.opencode` 自定义工具（连接器元工具照常 spawn oo，见 [conventions.md §7](conventions.md)）。

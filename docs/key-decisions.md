...
- **决策**：改为浏览器登录流（console launcher → deep-link 回跳 → authID 换 `oomol-token` 会话 token → profile 落盘 `auth.json` → `applyAuthAccount` 动态装配 agent）。完整 5 步与凭证细节见 [architecture.md §5](architecture.md)（现行流程的唯一权威描述）。

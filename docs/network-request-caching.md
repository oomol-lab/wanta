...
1. 缓存按数据身份划分，不能按页面外观划分。例如顶部浮层、购买弹窗和账单详情页共用账单资源；账单资源内部仍须区分组织计划/用量和创建者个人余额，不能把个人余额解释成组织钱包。
...
| Billing overview     | 账号（创建者余额/用量折扣订阅）、组织（Team 计划/用量）、统计周期 | 60 秒                                 | 充值或任一订阅/席位变更后强制刷新           |
...
`invalidateOrganizationDetailsResource(accountId, organizationId)`；已挂载的订阅者会立即收到失效通知并重新读取。
...
`/v1/apps` 成功时，组织连接摘要向当前组织成员展示，与连接管理权限分离；只有连接、重连、断开和配置等

...
> OpenConnector 服务。隔离目录已在无 PAT、无用户 `.npmrc`、无预存 `.oo-bin` 条件下完成 `npm ci`、
> 全质量门、production build、dev server 启动和 `prepare:binaries` 验证；打包中转目录确认包含可执行的
> oo、opencode 与 rg。公开发布前仍需完成 oo、内置 Skills 和两个 IPC 包的许可证/Notice 确认。
...
- 第三方可在构建时通过 `WANTA_ENDPOINT` 指向与当前 endpoint 约定兼容的自部署服务，oo CLI 继续作为
  OpenConnector 调用通道；
- `WANTA_OO_BIN` 只作为开发者覆盖路径；CI 可用 `OO_SKIP_BINARY_DOWNLOAD=1` 跳过不需要打包的下载步骤，
  但正式打包会由 `prepare:binaries` 重新确保二进制存在；
- oo CLI 与内置 Skills 的许可证和再分发边界仍须在公开发布前完成法务确认。

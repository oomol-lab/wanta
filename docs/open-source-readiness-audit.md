...
### 2.1 主代码许可证已落盘，配套声明仍缺失

- 等级：Legal Decision
- 当前事实：仓库根目录已有 Apache-2.0 `LICENSE`，`package.json` 的 `license` 字段与之相同；尚无
  `NOTICE`、`TRADEMARKS.md` 或 `THIRD_PARTY_NOTICES.md`。
- 风险：主代码授权已可见，但品牌资产、第三方二进制、Skills、字体、Logo 和依赖的再分发边界仍不能只靠
  主代码许可证覆盖。
- 决策：由授权负责人确认 Apache-2.0 主代码许可；单独确认 Wanta/OOMOL 名称、Logo、图标和协议标识的
  商标政策，并生成 Notice 与第三方声明。
- 验收：许可证和商标政策经授权负责人确认，配套文件进入根目录，package metadata 与之保持一致。

### 2.2 IPC 包已迁移到公共 npm

- 等级：Resolved / Verification
...
- 2026-07-20 以未携带 PAT 的 `npm view --registry=https://registry.npmjs.org` 验证两个精确版本及其
  tarball 均可公开读取。
- 剩余风险：两个包当前发布内容仍未声明 license；公开可下载不自动解决再分发许可证问题。
...
- 风险：官方包和社区自构建包的再分发权无法从仓库判断；默认安装与打包携带 oo 和导出 Skills，因此公开
  发布前必须解决许可证声明，不能用“运行时可不启用”替代再分发授权。
...
- 验收：fresh clone 无需预装 oo 即可由 `npm install` 准备完整开发环境；默认打包包含 oo；local runtime
  不实际调用 oo；公开发布物包含经确认的许可证和 Notice。
...
1. 验证默认安装与打包可公开下载并携带 oo CLI，同时保持 local runtime 不实际启用 Connector；

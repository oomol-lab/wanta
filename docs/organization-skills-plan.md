...
| 组织权限 Schema     | `src/api/organizations.ts`                   | `Organization` 返回 `role?: "creator" \| "member"` 与 `writable?: boolean`，权限优先读后端字段 |
...
- 组织 Skill 只对可管理组织的用户开放；权限优先使用组织 Schema 的 `writable`，没有该字段时再按 creator 推断。
- 组织 role 用于展示创建者/成员；后端返回 `role` 时优先使用，缺失时再按 `creator_user_id` 与 created 列表兼容旧响应。

...
| **oo-desktop** | 姊妹应用 + 工程化基线（独立仓库，不在本仓库内；本机路径因开发机而异）。Lumo 是新建独立仓库（不 fork），但构建/打包/CI/IPC 服务划分/UI 风格全部对齐 oo-desktop，保证两 App UI 不割裂、降低维护成本。注意已知差异：connector 鉴权头（Lumo 用 `Authorization: Bearer <会话 token>`，**不带** `x-oomol-user-uuid`；oo-desktop 用 auth.toml 账号 key 裸头——勿照抄）。                                                                                                                         |

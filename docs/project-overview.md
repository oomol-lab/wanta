...
| 组件           | 关系                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **OOMOL 云端** | LLM 网关（`https://llm.<endpoint>/v1`，OpenAI 兼容，模型名 `oomol-chat`）；connector 网关（`https://connector.<endpoint>`）；console（授权管理页）；hub（浏览器登录页）；api（账号 API）；static（自动更新分发）。全部由 `electron/domain.ts` 从单一 endpoint 派生。                                                                                                                                                                                                                     |
...
未变的锁定决策：Agent 内核 = OpenCode sidecar、连接器全经 oo CLI、模型名 `oomol-chat`、已授权清单来源 `/v1/apps`、三栏不可拖拽、deep-link `lumo://signin`（dev `lumo-local`）、IPC 用 `@oomol/connection`。

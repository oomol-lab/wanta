...
interprets it and orchestrates the selected Link runtime (OOMOL Connector, with 1,400+ popular apps and
10,000+ actions, or a user-configured OpenConnector) and local tools, streaming results back into the
chat area. Local capabilities are now enabled
...
- **UI shape**: three-pane layout — left: session navigation (create/delete/rename/switch across
  sessions; the settings entry lives here); center: the main content area, switching across seven shell
  routes (`archived` / `billing` / `chat` / `connections` / `teams` / `skills` /
...
- **Product surface beyond chat**: team workspaces (connector requests attach the
  ...
  the OpenCode workspace); and billing/usage pages.
...
| **OpenCode**      | The Agent kernel. Spawns the published binary `opencode-ai@1.18.30` as a sidecar; the main process drives it over HTTP+SSE via `@opencode-ai/sdk@1.18.30`. Pure configuration-level customization, zero source modification. At research time (2026-05) no community precedent was found for using OpenCode in non-IDE / general-agent workloads; Wanta was the first known case (positioned as a non-coding agent at inception; local coding capability was opened up later).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Custom tools were only `search_actions` / `call_action`                                                | Grew to four connector tools plus seven visible integrated-browser tools. Connector additions are `inspect_action` (enforcing inspect-before-call) and `list_apps` (directly lists connected apps in the active workspace).                                                                                                                                                                                                                                                                                                                                                                                           |

...
| **OpenCode**   | Agent 内核。spawn 已发布二进制 `opencode-ai@1.17.3`（sidecar），主进程经 `@opencode-ai/sdk@1.17.3` HTTP+SSE 驱动。纯配置级定制，零源码改动。调研时（2026-05）未发现 OpenCode 用于非 IDE/通用 agent 负载的社区先例，Lumo 是第一个已知案例（立项时定位非编码 agent，后放开了本地编码能力）。                                                                                                                                                                                               |

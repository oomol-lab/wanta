...
test("agent tool sources are present and shaped", () => {
  ...
  assert.ok(AGENT_TOOL_FILES["call_action.ts"]?.includes("do not claim the provider is unconnected"))
  assert.ok(AGENT_TOOL_FILES["call_action.ts"]?.includes("FAILED_PRECONDITION"))

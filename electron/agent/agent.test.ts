...
test("system prompt treats Link as a contextual capability, not the default path", () => {
  ...
  assert.match(WANTA_SYSTEM_PROMPT, /deleting `\/tmp` or `\/var\/tmp` themselves/)
  assert.match(WANTA_SYSTEM_PROMPT, /unrelated temporary directories are not automatically approved/i)

...
test("system prompt treats Link as a contextual capability, not the default path", () => {
  ...
  assert.match(WANTA_SYSTEM_PROMPT, /advisory discovery metadata only/)
  assert.match(WANTA_SYSTEM_PROMPT, /Never describe search_actions authenticated false/)
  assert.match(WANTA_SYSTEM_PROMPT, /Only call_action's authorization_required/)
...
test("agent tool sources are present and shaped", () => {
  ...
  assert.ok(AGENT_TOOL_FILES["search_actions.ts"]?.includes("advisory discovery metadata only"))
  assert.ok(AGENT_TOOL_FILES["search_actions.ts"]?.includes("does not prove that the task is blocked"))
  assert.ok(AGENT_TOOL_FILES["search_actions.ts"]?.includes("Only call_action authorization_required"))

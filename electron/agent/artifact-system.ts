export function buildArtifactSystem(artifactDir: string | undefined, outputProjectRoot?: string): string | undefined {
  ...
  return [
    ...
    "- Display generated HTML reports through Wanta's built-in artifact preview. Do not start a localhost HTTP server, call browser_navigate, or return a localhost URL merely to show a report. The integrated browser is for live websites or explicitly requested web-app testing.",
    "- Make HTML reports self-contained for the sandboxed artifact preview: inline CSS and classic JavaScript, embed images and fonts as data URLs, and include chart data in the file. External scripts, stylesheets, relative asset URLs, network requests, and nested frames are unavailable in this preview. Prefer inline SVG or embedded chart code over CDN libraries; do not start a server to work around missing report assets.",

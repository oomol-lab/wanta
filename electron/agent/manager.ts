...
function buildProcessSystem(processDir: string | undefined): string | undefined {
  ...
  return [
    ...
    "  - Common agent packages for spreadsheets, PDFs, documents, images, data analysis, and HTTP are normally approved automatically. Ordinary extras and version constraints are accepted; do not add a version constraint unless the task needs one.",
    ...
    "- When a task needs third-party Node.js modules only for temporary processing, install direct curated registry packages in this process directory with an explicit target such as `cd <process-directory> && npm install <package ...>`. Do not use global installation, package runners, custom registries, Git/URL/local sources, or user config.",

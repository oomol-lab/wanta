...
Do not call list_apps as a connection health check before normal SaaS reads or actions. It is an inventory and explicit-account-selection tool, not a prerequisite for execution. When the provider is already known and the user did not ask which account is connected, inspect and call the required action directly; let call_action's structured result determine whether authorization is blocked. A provider skill may document equivalent raw oo CLI commands, but inside Wanta prefer inspect_action and call_action because they preserve the current chat session's workspace identity and structured authorization handling.
...
- The active Wanta workspace is the connector identity. Link tools apply the current chat session's workspace identity automatically, including when the user switches the workspace from Wanta's sidebar. Raw oo CLI commands do not receive that per-session identity automatically. If a raw connector CLI command is truly necessary, use the exact workspace selector stated in the current-turn workspace context; never retry an organization failure by omitting its selector or by switching to personal identity. Prefer Link tools whenever they can perform the operation.

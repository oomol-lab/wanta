...
- When a current local project is supplied and dependency work is necessary, make the project target explicit: use a cd-to-project command followed by the package manager, or the package manager's explicit project-directory option. Direct packages from the standard registry are normally approved regardless of package popularity; package runners are ordinary local execution. Never use global installation, a custom registry, a user config, or a Git/URL/local package source unless the user explicitly asks.

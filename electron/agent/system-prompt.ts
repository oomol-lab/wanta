...
- In Default Access, use bash normally when it is the reliable path. Ordinary commands, scripts, project checks, data processing, concrete non-sensitive files, bounded directory work, and simple output filtering are expected to run without user-visible approval. In-scope dependency installation followed by ordinary checks or scripts in a straight-line && chain remains ordinary; you do not need to split these steps to avoid a permission prompt. Common launchers and package runners preserve the confirmation boundary of a recognized consequential operation.

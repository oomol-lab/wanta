...
- Structured execution status is the authority for connection blocking. Only call_action's authorization_required, or a matching skipped result with reason connection_blocked, establishes that execution is blocked pending access. Never describe search_actions authenticated false as "authorization required" or "connect to continue." Other errors describe action, provider, or runtime failures rather than proof that the account is disconnected. FAILED_PRECONDITION is a provider precondition failure unless the result also reports authorization_required.

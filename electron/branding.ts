...
export const branding = {
  ...
  storageKeyPrefix: "lumo",
  /** 自动更新 OSS/CDN 路径段（完整基址在 domain.ts 由 endpoint 派生，见 R2/阶段 6）。 */
  updateFeedPath: "release/apps/lumo",
} as const

/** 拼接一个 ServiceName 字符串，如 `serviceName("ping-service") === "lumo/ping-service"`。 */
export function serviceName(name: string): string {
  return `${branding.servicePrefix}/${name}`
}

/** 拼接一个带前缀的前端持久化 key，如 `storageKey("theme") === "lumo.theme"`。 */

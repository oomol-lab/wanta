...
export function buildPermissionModeSystem(mode: AgentPermissionMode | undefined): string {
  ...
  return [
    ...
    "- Wanta may pause only for basic safety boundaries such as credential/secret paths, broad home/system scans, private browser or Mail/Messages/Contacts/Calendars data, destructive deletion, global/system dependency changes, alternate package sources, privilege escalation, git push/reset/clean, publishing/deployment, or infrastructure mutations. Direct PyPI requirements in the task-private environment and direct standard-registry Node.js packages explicitly scoped to the task directory or current project are approved automatically regardless of package name, size, or runtime. Package runners are ordinary local execution. Other project dependency operations may be approved once for the current task.",

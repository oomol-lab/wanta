...
export function buildProjectContextSystem(project: ChatProjectContext | undefined): string | undefined {
  ...
  const lines = [
    ...
    "- For project dependency commands, make this directory explicit with `cd <project-directory> && <package-manager> ...` or the package manager's explicit project-directory option. Curated common direct Node.js dependencies can be approved automatically; other standard project dependency operations can receive a current-task approval. Do not use global installation, a custom registry, or a user config unless the user explicitly asks.",
...
export function buildPermissionModeSystem(mode: AgentPermissionMode | undefined): string {
  ...
  return [
    ...
    "- Wanta may pause only for basic safety boundaries such as credential/secret paths, broad home/system scans, private browser or Mail/Messages/Contacts/Calendars data, destructive deletion, unlisted or unsafe dependency installation, privilege escalation, git push/reset/clean, publishing/deployment, or infrastructure mutations. Curated common Python dependencies in the task-private environment and curated common direct Node.js dependencies explicitly scoped to the task directory or current project are approved automatically. Other standard project dependency operations may be approved once for the current task; global installs, custom registries, user configs, and Git/URL/local package sources remain protected.",

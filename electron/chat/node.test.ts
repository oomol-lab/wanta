...
test("dependency chains auto-reply through the event bridge while consequential suffixes remain pending", async () => {
  const bridge = createBridgeAgent()
  const projectPath = "/Users/example/code/wanta"
  const service = new ChatServiceImpl(bridge.agent, {
    projectStore: projectStore([
  ...
  const ordinary = "npm install && npm test && npm run build"
  bridge.emit({
    type: "permission.v2.asked",
    properties: {
      id: "chain-1",

...
export function resolveShellPath(value: string, cwd?: string): string | undefined {
  const isWindows = (path: string) =>
    /^[A-Za-z]:[\\/]/u.test(path) || path.startsWith("\\\\") || /^\/\/[^/]+\/[^/]+/u.test(path)
  const windows = isWindows(value) || isWindows(cwd ?? "")
  const slash = (path: string) => (windows ? path.replace(/\\/gu, "/") : path)
  let target = slash(value)
  ...
  if (windows && target.startsWith("/") && !root(target)) {
    const drive = base && /^[A-Za-z]:\//u.test(base) ? base.slice(0, 2) : undefined
    if (!drive || target.startsWith("//")) return undefined
    target = `${drive}${target}`
  }

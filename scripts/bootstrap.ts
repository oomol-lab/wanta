...
export function renderEnvScript(env: Record<string, string>): string {
  return [
    ...
    "# Source this file or run `corepack npm run dev:worktree`.",
...
async function main(args: string[]): Promise<void> {
  ...
  if (!skipInstall) {
    await run(commandName("corepack"), ["npm", "ci"], {})
  }
  await run(commandName("corepack"), ["npm", "run", "predev"], config.env)
  ...
  console.log("[wanta] run: corepack npm run dev:worktree")
}

async function createBootstrapConfig(): Promise<BootstrapConfig> {
...
async function writeBootstrapFiles(config: BootstrapConfig): Promise<void> {

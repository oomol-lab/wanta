...
test("resolveLarkCliTarget maps supported platforms and architectures to release assets", () => {
  assert.deepEqual(resolveLarkCliTarget("darwin", "x64"), {
    archiveKind: "tar.gz",
    assetName: "lark-cli-1.0.95-darwin-amd64.tar.gz",
    binaryName: "lark-cli",
  })
  assert.deepEqual(resolveLarkCliTarget("darwin", "arm64"), {
    archiveKind: "tar.gz",
    assetName: "lark-cli-1.0.95-darwin-arm64.tar.gz",
    binaryName: "lark-cli",
  })
  assert.deepEqual(resolveLarkCliTarget("linux", "riscv64"), {
    archiveKind: "tar.gz",
    assetName: "lark-cli-1.0.95-linux-riscv64.tar.gz",
    binaryName: "lark-cli",
  })
  assert.deepEqual(resolveLarkCliTarget("win32", "arm64"), {
    archiveKind: "zip",
    assetName: "lark-cli-1.0.95-windows-arm64.zip",

...
//   - 上游发布的平台包 tarball 内 bin/oo 是 0644（缺少可执行位，1.2.0 起、1.2.3 复核仍是），故提取后必须 chmod 0o755，
//     否则直接 spawn 会 EACCES——这正是改造前 dev 直连 node_modules 报错的根因。
//
// 平台映射取自 @oomol-lab/oo-cli 的 platform-targets.json（1.2.0；1.2.3 复核平台集未变），含 Linux glibc/musl 判别。
...
export const OO_CLI_VERSION = "1.2.3"

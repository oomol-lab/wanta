...
//                macOS 是 `mac-<arch>/Gimo.app`；Windows 是 `win-unpacked`
...
export async function collectMacMetadata({
  ...
  const dmgPath = path.join(releaseDir, `Gimo-${version}.dmg`)
  const zipPath = path.join(releaseDir, `Gimo-${version}.zip`)
  const appPath = path.join(releaseDir, macUnpackedDirName(arch), "Gimo.app")
...
export async function collectWinMetadata({
  ...
  const exePath = path.join(releaseDir, `Gimo-${version}-Setup.exe`)

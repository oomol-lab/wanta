...
export interface TeamAvatarStyle extends CSSProperties {
  backgroundColor: string
  borderColor: string
  color: string
}

export const teamAvatarPalette = [
...
] as const satisfies readonly TeamAvatarStyle[]
...
export function teamAvatarStyle(seed: string): TeamAvatarStyle {
  return teamAvatarPalette[stablePaletteIndex(seed)] ?? teamAvatarPalette[0]
}

function stablePaletteIndex(seed: string): number {
  const normalized = seed.trim().toLocaleLowerCase() || "team"
  let hash = 2166136261
  for (const character of normalized) {
    hash ^= character.codePointAt(0) ?? 0
    hash = Math.imul(hash, 16777619)
  }
  return (hash >>> 0) % teamAvatarPalette.length
}

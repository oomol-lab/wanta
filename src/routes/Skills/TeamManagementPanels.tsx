...
export function TeamSwitcherPanel({
  ...
  const { t } = useAppI18n()
  const countLabel = t("teams.teamCount", { count: teams.length })
            ...
            <div className="flex min-w-0 items-baseline gap-3">
              {selectedTeam ? (
                <>
                  ...
                  <TeamIdLabel teamId={selectedTeam.id} />
                </>
              ) : (
                <span className="oo-text-body min-w-0 truncate text-muted-foreground">{t("teams.selectTeam")}</span>
              )}
            </div>
          ...
          <div className="flex min-w-0 flex-wrap items-center justify-between gap-2 sm:justify-end">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" sideOffset={6} className="w-[min(36rem,calc(100vw-2rem))]">
                <DropdownMenuLabel>{t("teams.selectWorkspace")}</DropdownMenuLabel>
                <DropdownMenuSeparator />
                {teams.map((team) => {
...
function TeamIdLabel({ teamId }: { teamId: string }) {
  const { t } = useAppI18n()

  return (
    <span className="group/team-id inline-flex min-w-0 items-center gap-1.5 align-baseline">
      <span

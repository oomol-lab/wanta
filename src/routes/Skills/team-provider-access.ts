...
export type AppAccessParseResult =
  | { access: TeamAppAccess; grants: ProviderGrant[]; ok: true }
...
export function removeProviderGrant(access: TeamAppAccess, userId: string): TeamAppAccess {
  const next = cloneAppAccess(access)
  const subject = userSubject(userId)
  const services = next[subject]
  if (!services) {
...
function cloneAppAccess(access: TeamAppAccess): TeamAppAccess {

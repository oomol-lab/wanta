...
import type { BusyAction, TeamSkillLinkInput } from "./team-management-model.ts"
...
import { planProviderSkillRecommendationBulkLinks, runtimeSkillRemoveBusyKey } from "./team-management-model.ts"
...
function publicPackageLinkInput(pkg: PublicSkillPackage, skillName?: string): TeamSkillLinkInput | null {
...
export function useTeamSkillActions({
  busyAction,
  teamSkills,
  setBusyAction,
}: {
  ...
  const action = useScopedBusyAction({
    busyAction,
    contextKey: teamSkills?.teamId ?? "no-team",
    setBusyAction,
  })
  ...
  const removeRuntimeSkill = React.useCallback(async () => {
    ...
    try {
      ...
      if (action.isCurrent(operation)) {
        ...
        toast.success(t("teams.skillManageRemoveRuntimeSuccess", { name: target.displayName }))
      }
    } catch (error) {
      if (action.isCurrent(operation)) {
        toast.error(t("teams.skillManageRemoveRuntimeFailed", { error: skillErrorMessage(error, t) }))
      }
    } finally {
      action.finish(operation)
    }
    ...
    async (skills: readonly { packageName: string; skillName: string }[]) => {
      ...
      try {
        ...
        if (result.installed.length > 0) {
          skillVersionReport.invalidate()
          if (action.isCurrent(operation)) {
            toast.success(t("teams.skillManageInstallMissingSuccess", { count: result.installed.length }))
          }
        }
        if (result.failures.length > 0 && action.isCurrent(operation)) {
          toast.error(
            t("teams.skillManageInstallMissingFailed", {
  ...
  const addTeamSkillFromRecommendation = React.useCallback(
    async (recommendation: ProviderSkillRecommendation, options: { installRuntime: boolean }) => {
      ...
      try {
        ...
        if (action.isCurrent(operation)) {
          toast.success(t("teams.skillManageAddSuccess"))
          if (result.runtimeError) {
            toast.error(
              t("teams.skillManageRuntimeInstallPartialFailure", {
    ...
    [action, linkTeamSkill, teamSkills?.canManage, t],
  )

  const addTeamSkillFromPackage = React.useCallback(
    async (pkg: PublicSkillPackage, options: { installRuntime: boolean; skillName?: string }) => {
      ...
      if (!operation) return
      try {
        const result = await linkTeamSkill(input, options)
        if (action.isCurrent(operation)) {
          toast.success(t("teams.skillManageAddSuccess"))
          if (result.runtimeError) {
            toast.error(
  ...
  const addTeamSkillBatch = React.useCallback(
    async (recommendations: readonly ProviderSkillRecommendation[], options: { installRuntime: boolean }) => {
      if (!teamSkills?.canManage || recommendations.length === 0) {
        return
      }

      const plan = planProviderSkillRecommendationBulkLinks(recommendations, teamSkills.skills)
        ...
        for (const recommendation of plan.linkable) {
          try {
            await linkTeamSkill(
              ...
              { installRuntime: false, refreshTeam: false },
            )
            linkedCount += 1
        ...
        if (linkedCount > 0 && action.isCurrent(operation)) {
          toast.success(
            options.installRuntime && installedCount > 0
              ? t("teams.skillManageBulkAddInstallSuccess", { count: installedCount })
              : t("teams.skillManageBulkAddSuccess", { count: linkedCount }),
          )
        }
        if (failedCount > 0 && action.isCurrent(operation)) {
          toast.error(
            t("teams.skillManageBulkAddFailed", {
              count: failedCount,
    ...
    [action, linkTeamSkill, teamSkills, skillInventory, skillService, skillVersionReport, t],
  )
